...
$seenFile = $_ENV['SEEN_FILE'] ?? __DIR__ . '/seen.json';
...
if (file_exists($seenFile)) {
    try {
        ...
        fwrite(STDERR, "[" . date('c') . "] Error loading seen file: " . $e->getMessage() . PHP_EOL);
        $seen = [];
    }
}
...
function saveSeen($seen, $seenFile) {
    try {
        ...
        fwrite(STDERR, "[" . date('c') . "] Error saving seen file: " . $e->getMessage() . PHP_EOL);
    }
}
...
function fetchListings(Client $client, string $url): array {
    ...
    // Find anchors that contain "/oferta/" in the href (similar to JS version)
    $crawler->filter('a[href*="/oferta/"]')->each(function (Crawler $node) use (&$listings) {
        ...
        // Extract ID from URL (similar to JS regex)
        $id = null;
        ...
        // Title - try multiple selectors like JS version
        $title = '';
        try {
...
// Send to Discord webhook (similar to send.php but with dynamic data)
function notifyDiscord(string $webhookUrl, array $listing, Client $client) {
    ...
    if ($result === FALSE) {
        fwrite(STDERR, "[" . date('c') . "] Error sending webhook for listing {$listing['id']}\n");
        return false;
    } else {
        fwrite(STDOUT, "[" . date('c') . "] ✅ Sent notification for: {$listing['title']}\n");
        return true;
    }
}

// Main loop
fwrite(STDOUT, "[" . date('c') . "] Starting OLX sniper with webhooks. Polling {$searchUrl} every {$pollInterval}s\n");

while (true) {
    try {
        fwrite(STDOUT, "[" . date('c') . "] Polling " . $searchUrl . PHP_EOL);
        ...
        if (empty($listings)) {
            ...
            foreach ($listings as $listing) {
                if (!in_array($listing['id'], $seen)) {
                    $newCount++;
                    fwrite(STDOUT, "[" . date('c') . "] New listing: {$listing['title']} ({$listing['id']})\n");
                    ...
                    if ($success) {
                        ...
                        fwrite(STDERR, "[" . date('c') . "] Failed to notify Discord for {$listing['id']}\n");
                    }
                }
            }
            
            if ($newCount === 0) {
                fwrite(STDOUT, "[" . date('c') . "] No new listings found.\n");
            } else {
                fwrite(STDOUT, "[" . date('c') . "] Found {$newCount} new listings.\n");
            }
        }
    } catch (Exception $e) {
